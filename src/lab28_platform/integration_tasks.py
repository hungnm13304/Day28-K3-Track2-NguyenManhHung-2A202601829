...
def dedupe_latest(events: Iterable[IngestionEvent]) -> list[IngestionEvent]:
    ...
    """
    newest_by_key: dict[str, IngestionEvent] = {}
    for event in events:
        current = newest_by_key.get(event.idempotency_key)
        if current is None or (event.occurred_at, event.event_id) > (
            current.occurred_at,
            current.event_id,
        ):
            newest_by_key[event.idempotency_key] = event
    return [newest_by_key[key] for key in sorted(newest_by_key)]
